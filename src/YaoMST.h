  ...
  public:
	explicit YaoMST(const std::vector<Point2d> &points, double sFactor = 0.13)
	{
		...
		for (auto &tran : trans)
		{
			...
			std::sort(oldTree.begin(), oldTree.end(), [](const PointPair &lhs, const PointPair &rhs) { return lhs.nPoint.x < rhs.nPoint.x; });
			std::vector<Cell> cells;
			for (int i = 0; i < n; i += step1)
			{
				const int up = std::min(i + step1, n);
				std::sort(oldTree.begin() + i, oldTree.begin() + up, [](const PointPair &lhs, const PointPair &rhs) { return lhs.nPoint.y > rhs.nPoint.y; });
			}
			...
			for (int i = 0; i < n; i += step1)
			{
				...
				for (int j = i; j < up; j += step2)
					cells.push_back(
						{{oldTree.data() + j, oldTree.data() + std::min(j + step2, up)},
						 {newTree.data() + j, newTree.data() + std::min(j + step2, up)}});
			}
			for (auto &pointPair : oldTree)
			{
				...
				for (auto &cell : cells)
				{
					...
					{
						if (cell.oBlock.nearest(pointPair).dis2 < dis2)
							cell.nBlock.forEach([&](const PointPair &pr) {
								if (pr.index != pointPair.index && pr.nPoint.x >= pointPair.nPoint.x && pr.nPoint.y >= pointPair.nPoint.y)
									checkMin(pr.index, (pr.oPoint - pointPair.oPoint).length2());
							});
					}
				}
				if (to != -1)
